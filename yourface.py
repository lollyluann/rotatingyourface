...
model = tf.keras.models.load_model("/n/home01/dzhou/data/experiments/cp/best_model")

first_task = keras.Model(inputs=model.inputs,
                         outputs=model.layers[9].output)
first_task = tf.keras.models.load_model("/n/home01/dzhou/data/experiments/cp/best_first_task")
...
def loss_fn(input, output, cnn_model):
    ...
    # symmetry loss:
    # output is N x h x w x c
    # output_np = output.numpy()
    # sym_loss = output_np - np.flip(output_np, axis=2)
    # https://www.tensorflow.org/api_docs/python/tf/reverse

    # input = cv2.cvtColor(cv2.UMat(input), cv2.COLOR_BGR2GRAY)
    # input = tf.math.reduce_mean(input, axis=3)
    loss = 0
    ...
    for i in range(num_epochs):
        ...
        for batch in tqdm(train_ds):
            ...
            with tf.GradientTape() as tape:
                # Forward pass.
                output = model(batch)
                intermediate_output = first_task(batch)

                # Loss value for this batch.
                loss_value_1 = loss_fn(batch, intermediate_output, cnn_model)
                loss_value_2 = aux_task_loss(batch, output)
                loss = gamma*loss_value_1 + (1-gamma)*loss_value_2
